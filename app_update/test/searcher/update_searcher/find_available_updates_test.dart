  ...
  group('UpdateSearcher - findAvailableUpdates', () {
    ...
    test(
      ...
      () {
        final updates = [
          ...
          ), // другая платформа → skip
          UpdateSearcherTestUtils.createUpdateData(
            '2.0.0',
            date: DateTime(2024, 10, 10),
            platform: UpdatePlatform.android,
            source: UpdateSourceName.appStore,
          ), // другой источник → skip
          UpdateSearcherTestUtils.createUpdateData(
            '1.0.0',
            date: DateTime(2024, 10, 10),
            platform: UpdatePlatform.android,
            source: UpdateSourceName.googlePlay,
          ), // == local → ok, но будет отброшен, т.к. уже есть запись для пары
          UpdateSearcherTestUtils.createUpdateData(
            '0.9.0',
            date: DateTime(2024, 10, 10),
            platform: UpdatePlatform.android,
        ...
        expect(
          res.every((e) => e.sourceName == UpdateSourceName.googlePlay),
          isTrue,
        );
    ...
    test(
      ...
      () {
        ...
        expect(res.length, 1);
        expect(res.firstOrNull?.version, Version.parse('2.0.0'));
      },
    );
    ...
    test(
      ...
      () {
        ...
        expect(
          res.map((e) => e.version.toString()).toList(),
          ['2.1.0', '2.1.0', '1.9.0'],
        );
    ...
    test(
      'одинаковые версии сортируются по приоритету источников из sources',
      () {
        final updates = [
          UpdateSearcherTestUtils.createUpdateData(
            '2.0.0',
    ...
    test('источники, отсутствующие в списке sources, отфильтровываются', () {
      final updates = [
        UpdateSearcherTestUtils.createUpdateData(
          ...
          date: DateTime(2024, 10, 10),
          platform: UpdatePlatform.android,
          source: const UpdateSourceName.custom('custom'),
        ), // нет в списке
        UpdateSearcherTestUtils.createUpdateData(
          '2.0.0',
          date: DateTime(2024, 10, 10),
      ...
      expect(res.length, 1);
      expect(res.firstOrNull?.sourceName, UpdateSourceName.googlePlay);
    });
